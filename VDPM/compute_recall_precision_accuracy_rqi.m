...
function [recall, precision, accuracy, ap, aa] = compute_recall_precision_accuracy_rqi(cls, vnum_train, vnum_test)
...
correct_view = [];
theta_errors = 0:1:45;
correct_view_list = zeros(length(theta_errors), 76003); % IT's HARD CODED HERE for CHAIRS !!!!!!!!
% correct_view_list = zeros(length(theta_errors), 52097);
overlap = [];
count = zeros(M,1);
num = zeros(M,1);
num_pr = 0;
...
%                 if view_pr == view_gt(index)
                error = GetError((view_pr-1)*360/vnum_train, view_gt_360(index));
%                 if error <= theta_error
%                     correct_view(num_pr) = 1;
%                 else
%                     correct_view(num_pr) = 0;
%                 end
                for kk = 1:length(theta_errors)
                if error <= theta_errors(kk)
                    correct_view_list(kk, num_pr) = 1;
                end
                end
            else
                overlap{num_pr} = [];
...
correct_view = correct_view_list(kk,index);
n = numel(threshold);
recall = zeros(n,1);
precision = zeros(n,1);
accuracy = zeros(n,1);
...
save('chair_VDPM_AVPs', 'theta_errors', 'APs', 'AVPs');
...
% ---------------
function err = GetError(pred, label)
% ---------------
err = min(abs(pred-label), 360.0-abs(pred-label));
